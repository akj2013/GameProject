...
## 11. World Elevation Strategy (월드 높이 구조 설계)

WoodLand3D는 **자원 채집 중심 모바일 게임**이며  
타일 기반 월드 시스템을 사용한다.

이때 전체 월드를 완전한 3D 지형으로 만들면 시스템 복잡도가 급격히 증가한다.
...
월드는 **평면 타일 기반 시스템**을 유지한다.

하지만 **특정 랜드마크 지역에 한해 높이(Y축) 차이를 허용한다.**
...
월드의 높이는 **타일 단위가 아니라 구역 단위로 관리한다.**
...
├─ 정자 언덕 구역 (Y=3)
└─ 절 전망대 구역 (Y=6)
이렇게 **지역 단위 높이 차이**를 사용한다.

---

### 3. 왜 타일 단위 높이를 사용하지 않는가

타일마다 높이를 가지면 다음 문제가 발생한다.
...
따라서 **타일 시스템은 평면을 유지한다.**

---

### 4. 랜드마크 구역 설계

특정 구역은 다음 방식으로 높이를 만든다.
...
높이가 다른 구역은 다음 오브젝트로 연결한다.

- 계단 (Stairs)
- 경사로 (Ramp)
- 오르막 길 (Slope Path)

이 오브젝트는 **일반 Mesh + Collider** 로 구성한다.
평지 → 계단/오르막 → 언덕 마당 → 더 높은 계단 → 절/정자

---

### 6. 카메라 연출

특정 구역에서는 카메라 연출을 변경할 수 있다.
...
이 기능은 **Camera Zone 시스템**으로 구현할 예정이다.

---

### 7. 핵심 설계 요약

WoodLand3D 월드는 다음 구조를 따른다.
평면 타일 기반 월드
+
구역 단위 높이 시스템
